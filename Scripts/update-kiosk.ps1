...
$PatchFile = Join-Path $PatchHubPath "BLogicKiosk.zip"
$DownloadPath = Join-Path $PatchHubPath "BLogicKioskPatch"
$BLogicKioskPath = "C:\Program Files (x86)\BLogic Systems\BLogicKioskWeb"
...
function Prepare-Environment {
    if (-not (Test-Path $BLogicKioskPath)) {
        $result = "BLogicKiosk is not installed"
        Write-Host $result
        return $result
    }
...
function Install-Patch {
    ...
    Copy-Item -Path "$DownloadPath\*" -Destination $BLogicKioskPath -Recurse -Force
