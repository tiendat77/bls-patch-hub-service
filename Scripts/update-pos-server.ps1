...
$PatchFile = Join-Path $PatchHubPath "BLogicPosServer.zip"
$DownloadPath = Join-Path $PatchHubPath "BLogicPosServerPatch"
$BLogicPosServerPath = "C:\inetpub\wwwroot\BLogicService\bin"
...
function Prepare-Environment {
    if (-not (Test-Path $BLogicPosServerPath)) {
        $result = "POS Server is not installed"
        Write-Host $result
        return $result
    }
...
function Install-Patch {
    ...
    Copy-Item -Path "$DownloadPath\*" -Destination $BLogicPosServerPath -Recurse -Force
