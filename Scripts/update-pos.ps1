...
$PatchFile = Join-Path $PatchHubPath "BLogicPos.zip"
$DownloadPath = Join-Path $PatchHubPath "BLogicPosPatch"
$BLogicPosPath = "C:\Program Files (x86)\BLogic Systems\BLogic PointOfSale"
...
function Prepare-Environment {
    if (-not (Test-Path $BLogicPosPath)) {
        $result = "BLogicPos is not installed"
        Write-Host $result
        return $result
    }
...
function Install-Patch {
    ...
    Copy-Item -Path "$DownloadPath\*" -Destination $BLogicPosPath -Recurse -Force
