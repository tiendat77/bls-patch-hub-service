...
$PatchFile = Join-Path $PatchHubPath "BLogicStartKiosk.zip"
$DownloadPath = Join-Path $PatchHubPath "BLogicStartKioskPatch"
$BLogicStartKioskPath = "C:\Program Files (x86)\BLogic Systems\BLogic Start Kiosk"
...
function Prepare-Environment {
    if (-not (Test-Path $BLogicStartKioskPath)) {
        $result = "BLogicStartKiosk is not installed"
        Write-Host $result
        return $result
    }
...
function Install-Patch {
    ...
    Copy-Item -Path "$DownloadPath\*" -Destination $BLogicStartKioskPath -Recurse -Force
